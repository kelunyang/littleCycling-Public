/**
 * Route geometry utilities shared by the frontend renderer and the
 * server-side game simulation. Interpolates positions along a route from
 * pre-computed cumulative distances.
 *
 * Moved from packages/web/src/game/route-geometry.ts (map-specific helpers
 * `routeToGeoJSON`/`routeBounds` stayed behind — they depend on GeoJSON
 * types that only the web package needs).
 *
 * NOTE for the game simulation: always use
 * `cumulativeDistances[cumulativeDistances.length - 1]` as the route's total
 * distance (the same value `interpolateAlongRoute` clamps against), NOT an
 * independently-computed length — otherwise lap boundaries can drift a few
 * meters apart between the sim and the interpolator.
 */

import type { RoutePoint } from './types.js';
import { calcRouteDistance } from './gpx-parser.js';
